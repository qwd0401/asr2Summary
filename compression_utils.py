...
def cached_query(cache_instance: QueryCache, ttl: int = None):
    """
    查询缓存装饰器
    """
    def decorator(func):
        ...
        for stats in self.query_stats.values():
            ...
            if 'where' in query and stats['executions'] > 10:
                ...
                if 'transcription_id' in query:
                    suggestions.append("CREATE INDEX IF NOT EXISTS idx_transcription_id ON meeting_summaries(transcription_id);")
...
class CompressionStats:
    ...
    def record_compression(self, original_size: int, compressed_size: int, 
                          algorithm: str, data_type: str = 'text') -> None:
        """
        记录压缩统计
        """
        with self._lock:
            ...
            if data_type not in self.stats['compression_by_type']:
                self.stats['compression_by_type'][data_type] = {
                    'count': 0,
                    'original_size': 0,
                    'compressed_size': 0
                }
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        overall_ratio = (self.stats['total_compressed_size'] / 
