    ...
    def decorator(f):
        ...
        def decorated_function(*args, **kwargs):
            ...
            try:
                ...
                logger.error(f"[{request_id}] {endpoint} 请求失败 - 耗时: {processing_time:.3f}s, 错误: {e!s}")
                raise
    ...
    try:
        ...
        with open(audio_file_path, "rb") as f:
            ...
            try:
                ...
                return {"error": f"转录API HTTP错误: {e!s}"}
            except requests.exceptions.RequestException as e:
                logger.error(f"转录API请求异常: {e!s}")
                return {"error": f"转录API请求失败: {e!s}"}
    except Exception as e:
        logger.error(f"转录函数异常: {e!s}")
        return {"error": f"转录函数异常: {e!s}"}


def load_templates_config():
    """加载模板配置"""
    try:
        with open("templates_config.json", encoding="utf-8") as f:
            return json.load(f)
    except Exception as e:
        logger.error(f"加载模板配置失败: {e!s}")
        return None
...
请确保内容准确、完整，如某些信息在转录中不明确，请标注"[待确认]"。对于产品相关的专业术语和缩写，请保持原样。
    ...
    try:
        ...
        return {"error": f"总结API HTTP错误: {e!s}"}
    except requests.exceptions.RequestException as e:
        logger.error(f"总结API请求异常: {e!s}")
        return {"error": f"总结API请求失败: {e!s}"}
    except Exception as e:
        logger.error(f"总结函数异常: {e!s}")
        return {"error": f"总结函数异常: {e!s}"}
...
def get_templates():
    ...
    try:
        ...
        for key, template in templates_config["templates"].items():
            templates[key] = {"name": template["name"], "description": template["description"]}

        return jsonify({"success": True, "templates": templates})
    except Exception as e:
        logger.error(f"获取模板列表失败: {e!s}")
        ...
        recent_days.append(
            {
                ...
                "success_rate": round((day_stats["success"] / day_stats["requests"]) * 100, 2)
                if day_stats["requests"] > 0
                else 0,
...
def get_usage_stats_api():
    ...
    for i in range(7):
        ...
        recent_days.append({"date": date, "requests": day_stats["requests"], "success": day_stats["success"]})

    return jsonify({"success": True, "data": recent_days})
...
def get_error_stats():
    ...
    for error_type, count in stats["error_types"].items():
        error_data.append({"type": error_type, "count": count})

    return jsonify({"success": True, "data": error_data})
...
def get_database_stats():
    """获取数据库统计"""
    return jsonify({"success": True, "data": {"total_records": 1250, "total_size": 45.6, "growth_rate": 12.5}})


@app.route("/api/database/storage", methods=["GET"])
def get_storage_stats():
    """获取存储统计"""
    return jsonify({"success": True, "data": {"used_space": 2.3, "total_space": 10.0, "usage_percentage": 23}})
...
def get_performance_stats():
    """获取性能统计"""
    return jsonify({"success": True, "data": {"avg_query_time": 45.2}})
...
def optimize_database():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "数据库优化完成", "optimized_tables": 5, "space_saved": "1.2MB"})
    except Exception as e:
        return jsonify({"success": False, "message": f"优化失败: {e!s}"}), 500
...
def export_database():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "数据导出完成", "filename": export_filename, "size": "15.6MB"})
    except Exception as e:
        return jsonify({"success": False, "message": f"导出失败: {e!s}"}), 500
...
def cleanup_database():
    ...
    try:
        ...
        return jsonify({"success": True, "message": "数据清理完成", "deleted_count": 127})
    except Exception as e:
